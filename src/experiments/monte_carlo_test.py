...
class WithdrawType(Enum):
    FLAT=0        #set withdraw amount over the years ($50K in year 2025, $50K in year 2050)
...
class BasicWithdrawObj:
  def __init__(self, amount:float, inflation:float, withdrawTimingType:WithdrawTimingType):
...
class GuardRailWithdrawObj(BasicWithdrawObj):
  def __init__(self, amount:float, inflation:float, top_percent:float, bottom_percent:float,
...
class MonthlyDataObj:
  ...
  def get_date_data(self, year, month=1):
      ...
      assert _date in self._data, "file '%s' does not contain a date for '%s'" % (self._filename, _date)
  ...
  def _get_data(self, filename):
      ...
      with open(filename, "r") as f:
         _csv=csv.reader(f)
         for _date, value, _percent, in _csv:
...
class YearlyDataObj:
  ...
  def _get_data(self, filename):
      ...
      with open(filename, "r") as f:
         _csv=csv.reader(f)
         for _date, _returns, in _csv:
             self._data[_date]=float(_returns)


class BasicSimulator:
  def __init__(self, balance:float, withdrawObj:BasicWithdrawObj,
               #withdraw_amount:float, withdraw_inflation: float,
               start_year:int, length_in_years:int,
...
class BasicMonthlySimulator(BasicSimulator):
  def __init__(self, balance, monthly_withdraw, start_year, start_month,
               length_in_years=30, withdraw_timing=WithdrawTimingType.BEFORE):
      BasicSimulator.__init__(self, balance, monthly_withdraw, start_year,
                              length_in_years, withdraw_timing=withdraw_timing)
  ...
  def process(self):
      ...
      for _year in range(self._start_year, _end_year + 1):
          for _month in range(1, 13):
            ...
            if self._bankrupt:  #no need to do calcs below since we have no money left :(
...
class YearlySimulator(BasicSimulator):
  def __init__(self, balance, withdraw_obj, start_year, length_in_years=30):
      BasicSimulator.__init__(self, balance, withdraw_obj, start_year, length_in_years)
...
def stats(data):
    ...
    return {'min':min(data), 'max': max(data), 'sum': fsum(data), 'mean': st.mean(data),
            'median': st.median(data), 'std dev': st.stdev(data), 'last': data[-1]} 
