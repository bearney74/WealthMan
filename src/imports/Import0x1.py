...
from libs.GlobalVars import GlobalVars
from libs.Account import Account, AllocationPeriod, AssetClass, AssetClassPeriod
from libs.Person import Person
from libs.IncomeSources import IncomeSource, SocialSecurity
from libs.Expense import Expense
from libs.EnumTypes import (
    IncomeType,
    AmountPeriodType,
    AccountType,
...
class Import0x1(ImportHelper):
    ...
    def _get_globalvars_data(self):
        ...
        return GlobalVars(
            InflationRate=_inflation_rate,
            SocialSecurityCOLA=self._ss_cola,
            AssetWithdrawOrderByType=_ordertype,
            YearsToForecast=_years_to_forecast,
        ...
        for _person in _persons:
            ...
            if not _error_flag:
                if _dict["Num"] in _person_dict:
                    ...
                    _person_dict[_dict["Num"]] = Person(
                        Name=_dict["Name"],
                        BirthDate=self.str2date(_dict["BirthDate"]),
    ...
    def _get_income_employment_data(self, income_xml):
        ...
        for _emp in _emp_xml:
            ...
            _inc = IncomeSource(
                ...
                SurvivorPercent=self.strpct2float(_dict["SurvivorPercent"]),
                Taxable=_dict["Taxable"],
                Owner=_dict["Owner"],
            )
    ...
    def _get_income_pension_data(self, income_xml):
        ...
        for _emp in _emp_xml:
            ...
            _inc = IncomeSource(
                Name=_dict["Name"],
                IncomeSource=IncomeType.Pension,
                Amount=self.str2int(_dict["Amount"]),
                AmountPeriod=AmountPeriodType[_dict["AmountPeriod"]],
                BeginDate=_begin_date,
                EndDate=_end_date,
                SurvivorPercent=self.strpct2float(_dict["SurvivorPercent"]),
                COLA=self.strpct2float(_dict["COLA"]),
                Taxable=_dict["Taxable"],
                Owner=_dict["Owner"],
            )
    ...
    def _get_expense_data(self):
        ...
        for _exp in _exp_xml:
            ...
            _expense = Expense(
                ...
                Amount=self.str2int(_dict["Amount"]),
                AmountPeriod=AmountPeriodType[_dict["AmountPeriod"]],
                BeginDate=self.str2date(_dict["BeginDate"]),
                EndDate=self.str2date(_dict["EndDate"]),
                COLA=self.str2float(_dict["COLA"]),
            )
    ...
    def _get_assets_data(self):
        ...
        for _account in _account_xml:
            ...
            _acc = Account(
                Name=_dict["Name"],
                Type=_dict["Type"],
                Balance=self.str2int(_dict["Balance"]),
                Owner=_dict["Owner"],
                COLA=self.str2float(_dict["COLA"]),
            )
    ...
    def _get_account_allocation_periods(self, xml):
        ...
        for _period in _period_xml:
            ...
            # print(_dict)
            _ap = AllocationPeriod(
                Name=_dict["Name"],
                BeginDate=_dict["BeginDate"],
                EndDate=_dict["EndDate"],
                PercentStocks=_dict["PercentStocks"],
                PercentBonds=_dict["PercentBonds"],
    ...
    def _get_assetclass_data(self):
        ...
        for _period in _periods:
            ...
            _acp = AssetClassPeriod(
                _classes["Stocks"],
                _classes["Bonds"],
                _classes["MoneyMarket"],
                BeginDate=_begin_date,
                EndDate=_end_date,
            )
            _list.append(_acp)
