...
class IncomeExpenseBase:
  def __init__(self, Name:str, Amount: int, AmountPeriod:AmountPeriodType,
               BeginDate:date=None, EndDate:date=None, SurvivorPercent: float=None,
  ...
  def calc_balance_by_year(self, year) -> int:
    #this income source has no begin or end date..  so it should be calculated..
      ...
      if self.BeginDate is None:    #end date is not None
         ...
         self._annual_balance=int(_dh.percent_of_year()/100.0 * self._calc_balance())
         ...
         self._annual_income=int(_dh.percent_of_year()/100.0 * self._calc_balance())
         return self._annual_balance
